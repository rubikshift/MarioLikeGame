...
#define TEXT1_X_POS 10
#define TEXT1_Y_POS 10
#define TEXT2_X_POS 10
#define TEXT2_Y_POS 26
...
int main(int argc, char **argv)
{
	...
	SDL_SetColorKey(charset, true, 0);
	...
	mario* player = new mario(marioTexture, marioSpriteWidth, marioSpriteHeight, renderer);
	...
	while (!quit)
	{
		...
		else if ((gameLevels[actualLevel]->isFinished() && actualLevel == levels.size - 1) || player->lives == 0)
		{
			...
			drawString(screen, TEXT1_X_POS, TEXT1_Y_POS, text, charset);
			sprintf(text, "Esc - wyjscie, n - nowa gra, l - wczytaj gre");
			drawString(screen, TEXT2_X_POS, TEXT2_Y_POS, text, charset);
